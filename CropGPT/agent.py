...
from chnagr import start_Server
...
class Agent():
    ...
    def extract_parts(self,datasource):
        ...
        for path in datasource:
	        ...
	        if len(parts) > 2:
		        filename = parts[4]
#		        final_source.append(parts[3]+"\t"+"：《"+(os.path.splitext(filename)[0])+"》")
		        final_source.append("\t"+"：《"+(os.path.splitext(filename)[0])+"》")
    ...
    def query(self, query):
        tools = [
            ...
            Tool(
                ...
                description =  
               '该工具查询文献向量库，以解决较为广泛的玉米领域的论述类问题，例如：干旱胁迫对玉米生长的影响'
...
#          ),
            ...
            Tool(
                name = "CHN_AGR",
                func = lambda x: start_Server.queryTemp_Vdb(query),
                description = "查询中国农业科技信息网，回答玉米相关问题，若无法得出最终结果请使用其他工具"
            ),
#            Tool(
#                name = 'graph_zhu_func',
...
#                  description = '其他工具没有正确答案时，最后通过搜索引擎，可以回答通用类型的问题,例如，华中农业大学的最新科研成果',
#              ),
        ]
        ...
        prompt.template = """请用中文回答问题！Final Answer不能改变语义。
                        在回答玉米领域的论述类问题时，优先调用 CHN_AGR或者retrival_func 工具进行查询；
                        如果 retrival_func 无法提供相关结果或答案不够完善，再调用 CHN_AGR或者Science_func 工具进行查询
                        工具调用必须严格，必须明确各个工具的功能再进行调用，请注意工具一定不允许重复调用，不能出现随便调用的情况。
                        请记住你是华中农业大学信息学院智能机器人，你的身份不能出现通义千问!\n\n """ + prompt.template

#                        如果工具给出了相关信息，请总结给出一个简洁明了的Final Answer并立即结束思维链过程。
#                        Final Answer一定要简洁明了。
#        prompt.template = """请用中文回答问题！Final Answer不能改变语义。
